...
st.set_page_config(page_title="Malliavin Greeks for Macro Shock Sensitivity", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📐 Malliavin Greeks Engine</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Pathwise sensitivity of expected log return to macro shocks</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Malliavin Greeks")
...
st.sidebar.markdown(f"**Primary macro:** {config.PRIMARY_MACRO} | **Horizon:** {config.HORIZON}d")
st.sidebar.markdown(f"**Perturbation:** {config.PERTURBATION} | **Simulations:** {config.N_SIMULATIONS}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'malliavin_greeks_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Malliavin Greek: {etf['mg_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Greek"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Greek", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📐 Top ETFs by Malliavin Greek (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Malliavin calculus** provides pathwise derivatives of expectations with respect to shocks.
        - We estimate the sensitivity (Greek) of an ETF's expected log return to a 1% shock in the primary macro variable (e.g., VIX).
        - Method: fit GBM parameters to historical returns, regress returns on macro to estimate drift sensitivity (β), then simulate perturbed paths.
        - High Greek → ETF's expected return is highly sensitive to macro changes.
        - Positive Greek → macro shock increases expected return.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the Malliavin Greek scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Malliavin Greeks | Pathwise sensitivity to macro shocks")
