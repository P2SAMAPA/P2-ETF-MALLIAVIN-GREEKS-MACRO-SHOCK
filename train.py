...
from malliavin_greeks import malliavin_greek_full
...
def run_for_window(returns, macro_df, window_days):
    ...
    top_etfs = [{"ticker": t, "mg_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/malliavin_greeks_{timestamp}.json"
